...
export default ({
  ...
  const { preferences, chains, assets, gas_tokens_price, dev, wallet } =
    useSelector(
      (state) => ({
  ...
  useEffect(() => {
    if (!_.isEqual(transferData, data)) {
      ...
      const { origin_transacting_amount } = { ...data };
  ...
  const { explorer } = { ...source_chain_data };

  const { url, transaction_path } = { ...explorer };

  const source_asset_data = getAsset(
    null,
    assets_data,
  ...
  if (
    !source_contract_data &&
    equalsIgnoreCase(constants.AddressZero, origin_transacting_asset)
  ) {
  ...
  const source_asset_image =
    source_contract_data?.image || source_asset_data?.image;
  ...
  const _asset_data = getAsset(
    source_asset_data?.id,
    assets_data,
    destination_chain_data?.chain_id
  );
  const _contract_data = getContract(
    destination_chain_data?.chain_id,
    _asset_data?.contracts
  );
  const destination_asset_data = getAsset(
    null,
    assets_data,
    destination_chain_data?.chain_id,
    [
      ...
      _asset_data
        ? receive_local
          ? _contract_data?.next_asset?.contract_address
  ...
  if (
    !destination_contract_data &&
    equalsIgnoreCase(constants.AddressZero, destination_transacting_asset)
  ) {
    const { nativeCurrency } = {
  ...
  const destination_asset_image =
    destination_contract_data?.image || destination_asset_data?.image;
  const destination_amount = destination_transacting_amount
    ? Number(
        utils.formatUnits(
  ...
  const gas_token_data = toArray(gas_tokens_price_data).find((d) =>
    equalsIgnoreCase(d?.symbol, source_gas_native_token?.symbol)
  );

  const { price } = { ...gas_token_data };

  relayer_fee = relayer_fees
    ? _.sum(
        Object.entries(relayer_fees).map(
          ([k, v]) =>
            Number(
              utils.formatUnits(
                v,
                k === constants.AddressZero
                  ? source_gas_decimals
                  : source_decimals
              )
            ) *
            (relayerFeeAssetType === "transacting"
              ? k === constants.AddressZero
                ? price / source_asset_data?.price
                : 1
              : k === constants.AddressZero
              ? 1
              : source_asset_data?.price / price)
        )
      ).toFixed(
        relayerFeeAssetType === "transacting"
          ? source_decimals
  ...
  const calculateAmountReceived = async (_amount) => {
    if (sdk) {
      ...
      try {
        ...
        if (amount > 0) {
          console.log("[action required]", "[calculateAmountReceived]", {
            originDomain,
            destinationDomain,
            originTokenAddress,
            destinationTokenAddress,
        ...
        console.log("[action required]", "[calculateAmountReceived error]", {
          originDomain,
          destinationDomain,
          originTokenAddress,
          destinationTokenAddress,
      ...
      if (manual) {
        ...
        _estimatedValues = {
          ...
          originSlippage: "0",
          isNextAsset:
            typeof receive_local === "boolean" ? receive_local : false,
        };
      ...
      switch (error_status) {
        case XTransferErrorStatus.LowSlippage:
          try {
            ...
            console.log("[updateSlippage error]", {
              params,
              error,
            });
        ...
        case XTransferErrorStatus.LowRelayerFee:
          try {
            ...
            params = {
              ...
              transferId: transfer_id,
              asset: ["transacting"].includes(relayerFeeAssetType)
                ? origin_transacting_asset
                : constants.AddressZero,
              relayerFee: utils
                .parseEther(relayer_fee_to_bump || "0")
                .toString(),
            };

            console.log("[bumpTransfer]", {
              params,
            });
            ...
            console.log("[bumpTransfer error]", {
              params,
              error,
            });
      ...
      <Modal
        ...
        title={
          <div className="flex items-center justify-between">
            ...
            <div
              onClick={() => reset()}
              className="p-2 rounded-full cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800"
            >
              <BiX size={18} />
            </div>
          </div>
          ...
          <div className="mt-4 space-y-8">
            {error_status === XTransferErrorStatus.LowSlippage ? (
              <>
                <div className="space-y-3">
                  <div className="text-sm text-slate-600 dark:text-slate-400">
                    Your slippage tolerance is too low for this transfer to
                    complete under current market conditions.
                  </div>
                  <div className="text-sm text-slate-600 dark:text-slate-400">
                    You may increase your slippage tolerance below. If you take
                    no action, the transfer will continue to retry with your
                    current slippage tolerance.
                  </div>
                </div>
                <div className="flex flex-col space-y-3">
                  <div className="flex items-center justify-between space-x-1">
                    <div className="text-sm font-medium whitespace-nowrap text-slate-800 dark:text-slate-200">
                      Current slippage tolerance
                    </div>
              ...
              <>
                <div className="space-y-3">
                  <div className="text-sm text-slate-600 dark:text-slate-400">
                    The destination gas paid to complete this trade is currently
                    too low.
                  </div>
                  <div className="text-sm text-slate-600 dark:text-slate-400">
                    You may bump the amount of gas below for the relayer to
                    execute your transfer. If you take no action, the transfer
                    will continue to retry with the current amount.
                  </div>
                </div>
                <div className="flex flex-col space-y-3">
                  <div className="flex items-center justify-between space-x-1">
                  ...
                  <div className="flex items-center justify-between space-x-1">
                    ...
                    {!newRelayerFee && !estimateResponse ? (
                      <Oval width="20" height="20" color={loaderColor(theme)} />
                    ) : (
                      <span className="whitespace-nowrap text-slate-800 dark:text-slate-200 font-semibold space-x-1.5">
                        <DecimalsFormat
                          value={
                            relayer_fee_to_bump && relayer_fee_to_bump > 0
                              ? relayer_fee_to_bump
                              : 0
                          }
                          className="text-sm"
                        ...
                        {is_staging ? (
                          <select
                            ...
                            className="border-0 rounded bg-slate-100 dark:bg-slate-800 focus:ring-0"
                          >
                            {RELAYER_FEE_ASSET_TYPES.map((t, i) => {
                              return (
                                <option key={i} title={`${t} asset`} value={t}>
                                  {t === "transacting"
                                    ? source_symbol
                                    : source_gas_native_token?.symbol}
                                </option>
            ...
            ((["string", "number"].includes(
              typeof (error_status === XTransferErrorStatus.LowSlippage
                ? newSlippage
                : error_status === XTransferErrorStatus.LowRelayerFee
                ? newRelayerFee
                : null)
              ...
              wrong_chain ? (
                <Wallet
                  ...
                  className="w-full rounded-xl text-black bg-[#14f094] hover:bg-[#00CC66] dark:[#14f094] dark:hover:bg-[#00CC66]  flex items-center justify-cente text-base font-medium space-x-1.5 sm:space-x-2 py-3 sm:py-4 px-2 sm:px-3"
                >
              ...
              <button
                disabled={true}
                className="w-full px-2 py-3 text-base text-center rounded cursor-not-allowed bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 sm:py-4 sm:px-3"
              >
                Apply
              </button>
            ) : (
