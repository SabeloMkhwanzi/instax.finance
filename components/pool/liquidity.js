...
const DEFAULT_POOL_SLIPPAGE_PERCENTAGE = Number(
...
export default ({ pool, userPoolsData, onFinish }) => {
  const { preferences, chains, pool_assets, pools, dev, wallet, balances } =
    useSelector(
      (state) => ({
  ...
  const [priceImpactRemoveResponse, setPriceImpactRemoveResponse] =
    useState(null);
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      if (
        ...
        try {
          _amountX = utils
            .parseUnits((amountX || 0).toString(), adopted?.decimals || 18)
            .toString();

          _amountY = utils
            .parseUnits((amountY || 0).toString(), local?.decimals || 18)
            .toString();
          ...
          console.log("[calculateAddLiquidityPriceImpact error]", {
            domainId,
            contract_address,
            _amountX,
            _amountY,
  ...
  useEffect(() => {
    switch (withdrawOption) {
      ...
      case "y_only":
        ...
        setRemoveAmounts(
          [amount * ratio, amount * (1 - ratio)].map((a, i) =>
            isNaN(a)
              ? ""
              : removeDecimal(
                  a.toFixed((i === 0 ? adopted : local)?.decimals || 18)
                )
          )
        );
        setCallResponse(null);
    ...
    if (sdk) {
      ...
      switch (action) {
        case "deposit":
          ...
          if (!failed) {
            try {
              ...
              const tokenIndex = await sdk.sdkPool.getPoolTokenIndex(
                domainId,
                contract_address,
                contract_address
              );

              console.log("[poolTokenIndex]", {
                domainId,
                contract_address,
                tokenAddress: contract_address,
                tokenIndex,
              ...
              const add_request = await sdk.sdkPool.addLiquidity(
                domainId,
                contract_address,
                amounts,
                minToMint,
        ...
        case "withdraw":
          ...
          const _amount = utils
            .parseUnits((amount || 0).toString(), 18)
            .toString();
          ...
          if (!failed) {
            try {
              ...
              const remove_request = await sdk.sdkPool.removeLiquidity(
                domainId,
                contract_address,
                _amount,
                minAmounts,
    ...
    if (typeof amount === "string") {
      if (utils.parseUnits(amount || "0", 18).toBigInt() <= 0) {
        ...
        try {
          ...
          let amounts = await sdk.sdkPool.calculateRemoveSwapLiquidity(
            domainId,
            contract_address,
            _amount
          );

          console.log("[amountsRemoveSwapLiquidity]", {
            domainId,
            contract_address,
            amount: _amount,
            amounts,
          ...
          console.log("[calculateRemoveSwapLiquidity error]", {
            domainId,
            contract_address,
            amount: _amount,
            error,
  ...
  const calculateAddLiquidityPriceImpact = async (
    ...
    try {
      ...
      if (
        [chain_data?.id].includes(pool_data?.chain_data?.id) &&
        pool_data?.tvl
      ) {
        console.log("[calculateAddLiquidityPriceImpact]", {
          domainId,
          contractAddress,
          amountX,
          amountY,
        ...
        const price_impact = await sdk.sdkPool.calculateAddLiquidityPriceImpact(
          domainId,
          contractAddress,
          amountX,
          amountY
        ...
        console.log("[addLiquidityPriceImpact]", {
          domainId,
          contractAddress,
          amountX,
          amountY,
  ...
  const calculateRemoveLiquidityPriceImpact = async (
    ...
    try {
      ...
      if (
        ...
        console.log("[calculateRemoveLiquidityPriceImpact]", {
          domainId,
          contractAddress,
          amountX,
          amountY,
        ...
        const price_impact =
          await sdk.sdkPool.calculateRemoveLiquidityPriceImpact(
            domainId,
            contractAddress,
            amountX,
            amountY
        ...
        console.log("[removeLiquidityPriceImpact]", {
          domainId,
          contractAddress,
          amountX,
          amountY,
      ...
      console.log("[calculateRemoveLiquidityPriceImpact error]", {
        domainId,
        contractAddress,
        amountX,
        amountY,
  ...
  const { asset_data, contract_data, symbol, lpTokenAddress, error } = {
    ...pool_data,
  };
  ...
  const x_remove_amount = equalsIgnoreCase(adopted?.address, contract_address)
    ? _.head(removeAmounts)
    : _.last(removeAmounts);

  const y_remove_amount = equalsIgnoreCase(adopted?.address, contract_address)
    ? _.last(removeAmounts)
    : _.head(removeAmounts);

  const position_loading =
    selected && !no_pool && !error && (!userPoolsData || pool_loading);
  ...
  const native_asset = !adopted?.symbol?.startsWith(WRAPPED_PREFIX)
    ? adopted
    : local;

  const wrapped_asset = adopted?.symbol?.startsWith(WRAPPED_PREFIX)
    ? adopted
    : local;
  ...
  const valid_amount =
    action === "withdraw"
      ? typeof amount === "string" &&
        !isNaN(amount) &&
        amount &&
        utils.parseUnits(amount, 18).toBigInt() <=
          utils.parseUnits((lpTokenBalance || 0).toString(), 18).toBigInt() &&
      ...
      : typeof amountX === "string" &&
        ...
        (amountX || amountY) &&
        utils
          .parseUnits(amountX || "0", x_asset_data?.decimals || 18)
          .toBigInt() <=
          utils
            .parseUnits(
            ...
            .toBigInt() &&
        utils
          .parseUnits(amountY || "0", y_asset_data?.decimals || 18)
          .toBigInt() <=
          utils
            .parseUnits(
  ...
  const overweighted_asset =
    ...
    Number(amountX) +
      Number(
        (equalsIgnoreCase(adopted.address, x_asset_data?.contract_address)
          ? adopted
          : local
        ).balance
      ) >
      Number(amountY) +
        Number(
          (equalsIgnoreCase(adopted.address, y_asset_data?.contract_address)
            ? adopted
            : local
          ).balance
        )
      ...
      <div className="px-4 pt-4 pb-5 space-y-3 border bg-slate-50 dark:bg-slate-900 rounded-xl dark:border-slate-800">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between space-x-4 border-b w-fit dark:border-slate-800">
            {ACTIONS.map((a, i) => (
              <div
                ...
                } cursor-pointer capitalize text-sm text-left py-3 px-0`}
              >
                {a}
              </div>
            ))}
          ...
          {action === "deposit" ? (
            <>
              <div className="px-0 pt-1">
                <div className="space-y-1">
                          ...
                          <span
                            className={`${
                              disabled
                                ...
                                : "cursor-pointer text-green-400 hover:text-green-500 dark:text-green-500 dark:hover:text-gree-400"
                            } text-xs font-medium`}
                          >
                            Max
                          </span>
                  ...
                  <div className="space-y-1">
                    <div className="rounded-xl border dark:border-slate-800 flex items-center justify-between space-x-2 py-2.5 px-3">
                      {x_asset_data?.contract_address && (
                        <div className="flex items-center justify-between space-x-2">
                      ...
                      <DebounceInput
                        ...
                        disabled={disabled}
                        value={
                          ["string", "number"].includes(typeof amountX) &&
                          !isNaN(amountX)
                            ? amountX
                            : ""
                        }
                        ...
                        } border-0 focus:ring-0 text-base font-medium text-right`}
                      />
                    </div>
                    {typeof amountX === "string" &&
                      ["string", "number"].includes(typeof x_balance_amount) &&
                      utils
                        .parseUnits(
                          amountX || "0",
                          x_asset_data?.decimals || 18
                        ...
                        utils
                          ...
                          .toBigInt() && (
                        <div className="flex items-center justify-end space-x-1 text-red-600 dark:text-yellow-400 sm:mx-0">
                        ...
                        >
                          ...
                          <span
                            className={`${
                              disabled
                                ? "cursor-not-allowed text-slate-400 dark:text-slate-500"
                                : "cursor-pointer text-green-400 hover:text-green-500 dark:text-green-500 dark:hover:text-blue-400"
                            } text-xs font-medium`}
                          >
                            Max
                          </span>
                  ...
                  <div className="space-y-1">
                    <div className="rounded-xl border dark:border-slate-800 flex items-center justify-between space-x-2 py-2.5 px-3">
                      ...
                      <DebounceInput
                        ...
                        disabled={disabled}
                        value={
                          ["string", "number"].includes(typeof amountY) &&
                          !isNaN(amountY)
                            ? amountY
                            : ""
                        }
                    ...
                    {typeof amountY === "string" &&
                      ["string", "number"].includes(typeof y_balance_amount) &&
                      utils
                        .parseUnits(
                          amountY || "0",
                          y_asset_data?.decimals || 18
              ...
              <div className="space-y-4">
                {pool_data && (
                  <div className="flex flex-col space-y-3">
                    <div className="text-xs font-medium whitespace-nowrap text-slate-400 dark:text-slate-500">
                      Pool Ratio
                    </div>
                    <div className="w-full h-6 flex flex-col items-end justify-center space-y-1.5">
                      <ProgressBar
                        ...
                        style={{
                          backgroundColor: "#14f094",
                        }}
                        backgroundStyle={{
                          backgroundColor: "#112727 ",
                        }}
                      />
                              ...
                              <Image
                                src={
                                  wrapped_asset?.contract_data?.next_asset
                                    ?.image || wrapped_asset?.asset_data?.image
                                }
                                width={16}
                                height={16}
                                className="rounded-full"
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between space-x-1">
                    <Tooltip
                      ...
                      className="z-50 text-xs text-white w-80 bg-dark"
                    >
                  ...
                  {typeof priceImpactAdd === "number" && priceImpactAdd < 0 && (
                    <div className="flex items-start px-2 pt-2 pb-3 space-x-2 bg-opacity-50 rounded-xl bg-yellow-50 dark:bg-yellow-200 dark:bg-opacity-10">
                      <IoWarning
                        size={18}
                        className="mt-1 text-yellow-500 min-w-max dark:text-yellow-400"
                      />
                      <div className="flex flex-col space-y-3">
                        <div className="flex flex-col space-y-2.5">
                          <span className="text-base font-bold">Warning</span>
                          <span className="flex flex-wrap items-center text-xs leading-4 text-left">
                          ...
                          <div className="flex flex-col items-center space-y-1">
                            <span className="text-xs leading-4 text-left">
                              <span className="mr-1">
                              ...
                              <span className="mr-1 font-bold">bonus</span>
                              <span>tokens.</span>
                            </span>
                          </div>
                        </div>
                        <div className="flex flex-col space-y-0">
                          <div className="w-fit   bg-[#14f094] hover:bg-[#00CC66] dark:[#14f094] dark:hover:bg-[#00CC66] rounded-xl flex items-center space-x-1 pt-0.5 pb-1 px-2">
                            <a
                              href={`/${asset.toUpperCase()}-from-${
                                _.head(chains_data)?.id
                              }-to-${chain}?${
                                asset_data?.symbol ===
                                _.head(_.head(chains_data)?.provider_params)
                              ...
                              rel="noopener noreferrer"
                            >
                              <span className="text-xs font-medium leading-3 text-black">
                                Click here to get{" "}
                                {
                                  (overweighted_asset === "x"
                                    ? y_asset_data
              ...
              <div className="flex items-end">
                {!valid_amount ? (
                  <Button
                    disabled={true}
                    className="w-full px-2 py-3 text-base text-center cursor-not-allowed pointer-events-none rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 sm:px-3"
                  >
                  ...
                  <Wallet
                    ...
                    className="w-full rounded-xl text-black bg-[#14f094] hover:bg-[#00CC66] dark:[#14f094] dark:hover:bg-[#00CC66] flex items-center justify-center text-base font-medium space-x-1.5 sm:space-x-2 py-3 px-2 sm:px-3"
                  >
                  ...
                  <button
                    ...
                    className={`w-full ${
                      disabled || !valid_amount
                        ...
                        : "bg-green-500 hover:bg-green-600 dark:bg-green-600 dark:hover:bg-green-700 cursor-pointer text-white"
                    }  rounded-xl text-base text-center py-3 px-2 sm:px-3`}
                  >
                    <span className="flex items-center justify-center space-x-1.5">
                      {(calling || approving) && (
                  ...
                  <Wallet
                    ...
                    className="w-full px-2 py-3 text-base font-medium text-center rounded-xl text-black bg-[#14f094] hover:bg-[#00CC66] dark:[#14f094] dark:hover:bg-[#00CC66] sm:px-3"
                  >
            ...
            <>
              <div className="px-0 py-3 space-y-6">
                <div className="space-y-2">
                  <div className="flex items-center justify-between space-x-2">
                    <span className="text-xs font-medium">
                      Withdraw percentage
                  ...
                  <div className="space-y-1">
                    <div className=" rounded-xl border dark:border-slate-800 flex items-center justify-between space-x-1 py-2.5 px-3">
                      <DebounceInput
                        debounceTimeout={750}
                        size="small"
                        type="number"
                        ...
                        value={
                          ["string", "number"].includes(
                            typeof withdrawPercent
                          ) && !isNaN(withdrawPercent)
                            ? withdrawPercent
                            : ""
                        }
                        onChange={(e) => {
                    ...
                    {typeof amount === "string" &&
                      ...
                      utils.parseUnits(amount || "0", 18).toBigInt() >
                        utils
                          .parseUnits((lpTokenBalance || 0).toString(), 18)
                          .toBigInt() && (
                        <div className="flex items-center justify-end space-x-1 text-red-600 dark:text-yellow-400 sm:mx-0">
                          <BiMessageError size={16} className="min-w-max" />
                          <span className="text-xs font-medium">
                          ...
                          <div
                            ...
                            className={`${
                              ...
                              !["string", "number"].includes(
                                ...
                                ? "bg-slate-100 dark:bg-slate-800 pointer-events-none cursor-not-allowed text-green-400 dark:text-slate-200 font-semibold"
                                : FixedNumber.fromString(
                                    (lpTokenBalance || 0).toString()
                                  )
                                    .mulUnsafe(
                                    ...
                                    .toString() === amount
                                ? "bg-slate-300 dark:bg-slate-700 cursor-pointer text-green-600 dark:text-white font-semibold"
                                : "bg-slate-100 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-800 cursor-pointer text-green-400 dark:text-slate-200 hover:text-greem-600 dark:hover:text-white font-medium"
                            }  rounded-xl text-xs py-0.5 px-1.5`}
                          >
                            {p * 100} %
                          </div>
                        ))}
                  ...
                  <div className="space-y-4">
                    <div className="flex flex-col space-y-2">
                      {WITHDRAW_OPTIONS.map((o, i) => {
                        ...
                        title = title
                          .replace("{x}", x_asset_data.symbol)
                          .replace("{y}", y_asset_data.symbol);
                    ...
                    <div className="space-y-2.5">
                      <div className="space-y-2">
                        ...
                        <div className="bg-slate-100 dark:bg-slate-900  rounded-xl border dark:border-slate-800 space-y-0.5 py-2.5 px-3">
                          <div className="flex items-center justify-between space-x-2">
                            ...
                            {withdrawOption === "custom_amounts" ? (
                              ...
                              <DecimalsFormat
                                ...
                                className="text-base font-medium text-right bg-transparent w-fit text-slate-500 dark:text-slate-500"
                              />
                            ) : (
                              selected &&
                              !no_pool &&
              ...
              <div className="flex items-end">
                {!valid_amount ? (
                  ...
                  <Wallet
                    connectChainId={chain_id}
                    className="w-ful text-black bg-[#14f094] hover:bg-[#00CC66] dark:[#14f094] dark:hover:bg-[#00CC66]  rounded-xl flex items-center justify-cente text-base font-medium space-x-1.5 sm:space-x-2 py-3  px-2 sm:px-3"
                  >
                    <span className="mr-1.5 sm:mr-2">
                      {is_walletconnect ? "Reconnect" : "Switch"} to
                    </span>
                        ...
                        <div className="flex items-center justify-between space-x-2">
                          ...
                          <div className="flex items-center space-x-1">
                            ...
                            {status === "failed" ? (
                              ...
                              <button
                                onClick={() => reset()}
                                className="flex items-center justify-center p-1 text-black bg-green-500 rounded-full dark:bg-green-400"
                              >
                                <MdClose size={14} />
                              </button>
                            ) : null}
          ...
          {(_x_asset_data?.mintable ||
            ...
            <Faucet
              ...
              titleClassName={
                wrong_chain ? "text-slate-400 dark:text-slate-600" : ""
              }
              className="flex flex-col items-center justify-center w-full max-w-lg mx-auto space-y-2 bg-transparent"
            />
          )}
        </div>
      </div>
