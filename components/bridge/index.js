...
export default () => {
  ...
  const { chain_id, provider, browser_provider, signer, address } = {
    ...wallet_data,
  };
  ...
  const [relayerFeeAssetType, setRelayerFeeAssetType] = useState(
    _.head(RELAYER_FEE_ASSET_TYPES)
  );
  ...
  useEffect(() => {
    ...
    if (bridge) {
      const { source_chain, destination_chain, asset, symbol, amount } = {
        ...bridge,
      };
      ...
      if (destination_chain_data) {
        ...
        if (
          asset &&
          getAsset(asset, assets_data, destination_chain_data.chain_id)
        ) {
          params.asset = asset;
        }
      }
      ...
      if (params.source_chain && params.asset) {
        ...
        if (
          symbol &&
          getAsset(asset, assets_data, source_chain_data.chain_id, symbol)
        ) {
      ...
      toArray(router_asset_balances_data?.[chain_id])
        .filter(
          (a) =>
            ...
            ).findIndex((_a) => equalsIgnoreCase(a?.contract_address, _a)) > -1
        )
        .map((a) =>
          Number(
            utils.formatUnits(
    ...
    setOptions({
      ...
      slippage,
      forceSlow:
        destination_chain_data && router_asset_balances_data
          ? Number(amount) > routers_liquidity_amount
          : false,
      receiveLocal,
    });
  ...
  useEffect(() => {
    ...
    if (asPath && id) {
      if (
        !(source_chain && destination_chain) &&
        !equalsIgnoreCase(id, destination_chain)
      ) {
        const params = paramsToObj(
          asPath?.indexOf("?") > -1 && asPath.substring(asPath.indexOf("?") + 1)
        );
        ...
        if (
          ...
          getChain(id, chains_data, true)
        ) {
          source_chain = id;
        }
      } else if (
        !asPath.includes("from-") &&
        !equalsIgnoreCase(id, source_chain)
      ) {
        source_chain = id;
      }
    ...
    if (Object.keys(bridge).length > 0 || ["/"].includes(asPath)) {
      source_chain =
        source_chain ||
        getChain(null, chains_data, true, false, true, destination_chain)?.id;
      destination_chain =
        destination_chain && !equalsIgnoreCase(destination_chain, source_chain)
          ? destination_chain
          : bridge.source_chain &&
            !equalsIgnoreCase(bridge.source_chain, source_chain)
          ? bridge.source_chain
  ...
  useEffect(() => {
    const update = () => {
      if (
        estimateFeesTrigger &&
        !(approving || approveResponse || calling || xcall || xcallResponse)
      ) {
        estimateFees();
      }
    };
        ...
        if (!transfer_id && transactionHash) {
          ...
          try {
            const response = toArray(
              await sdk.sdkUtils.getTransfers({ transactionHash })
            );
            transfer_data = response.find((t) =>
              equalsIgnoreCase(t?.xcall_transaction_hash, transactionHash)
            );
          } catch (error) {}
          ...
          const response = toArray(
            await sdk.sdkUtils.getTransfers({ transferId: transfer_id })
          );
          const transfer_data = response.find((t) =>
            equalsIgnoreCase(t?.transfer_id, transfer_id)
          );

          const { relayer_fee, slippage, status, error_status } = {
            ...transfer_data,
          };
  ...
  useEffect(() => {
    const update = (is_interval) => {
      if (
        (openTransferStatus ||
          (latest_transfer && !latest_transfer.execute_transaction_hash)) &&
        (!timeTrigger || is_interval)
      ) {
        setTimeTrigger(!timeTrigger);
      }
    };
  ...
  const estimateFees = async () => {
    if (checkSupport() && !xcall && !xcallResponse) {
      ...
      const source_contract_data = getContract(
        source_chain_data.chain_id,
        source_asset_data?.contracts
      );
      const destination_contract_data = getContract(
        destination_chain_data.chain_id,
        ...
        if (sdk) {
          ...
          try {
            ...
            const routerFee = forceSlow
              ? 0
              : parseFloat(
                  ((Number(amount) * ROUTER_FEE_PERCENT) / 100).toFixed(
            ...
            const params = {
              ...
              priceIn: ["transacting"].includes(relayerFeeAssetType)
                ? "usd"
                : "native",
            };
            ...
            try {
              ...
              console.log("[estimateRelayerFee error]", params, {
                error,
              });
  ...
  const calculateAmountReceived = async (_amount, receive_local) => {
    if (sdk) {
      ...
      let destinationTokenAddress =
        _destination_contract_data?.contract_address;
      ...
      const amount = utils
        .parseUnits((_amount || 0).toString(), source_decimals)
        .toBigInt();
      ...
      try {
        ...
        if (
          ...
          toArray(
            ...
            toArray(pools_data).find(
              (p) => p?.chain_data?.id === chain && !p.tvl
            )
          ) < 0
        ) {
          console.log("[calculateAmountReceived]", {
            originDomain,
            destinationDomain,
            originTokenAddress,
            destinationTokenAddress,
          ...
          const response = await sdk.sdkBase.calculateAmountReceived(
            originDomain,
            destinationDomain,
            originTokenAddress,
            amount.toString(),
        ...
        console.log("[calculateAmountReceived error]", {
          originDomain,
          destinationDomain,
          originTokenAddress,
          destinationTokenAddress,
      ...
      if (manual) {
        const routerFee = parseFloat(
          ((Number(_amount) * ROUTER_FEE_PERCENT) / 100).toFixed(
            source_decimals
          )
        );
        ...
        setEstimatedValues({
          ...
          routerFee,
          isNextAsset:
            typeof receive_local === "boolean" ? receive_local : receiveLocal,
        });
      }
    }
  };
    ...
    if (sdk) {
      ...
      try {
        ...
        if (amount > 0) {
          ...
          const response = await sdk.sdkBase.approveIfNeeded(
            domain_id,
            contract_address,
            amount.toString()
          );
        ...
        console.log("[approveIfNeeded error]", {
          domain_id,
          contract_address,
          amount,
          error,
  ...
  const call = async (relayerFee = fees?.relayerFee) => {
    ...
    if (sdk) {
      const { source_chain, destination_chain, asset, amount, receive_wrap } = {
        ...bridge,
      };
      let { symbol } = { ...bridge };
      ...
      let source_contract_data = getContract(
        source_chain_data?.chain_id,
        source_asset_data?.contracts
      );
      ...
      if (symbol) {
        // next asset
        if (
          equalsIgnoreCase(source_contract_data?.next_asset?.symbol, symbol)
        ) {
          source_contract_data = {
            ...source_contract_data,
        ...
        // native asset
          ...
          equalsIgnoreCase(source_asset_data?.symbol, symbol)
        ) {
          source_contract_data = {
            ...source_contract_data,
      ...
      let destination_contract_data = getContract(
        destination_chain_data?.chain_id,
        destination_asset_data?.contracts
      );
      ...
      const relayer_fee_field = `relayerFee${
        relayerFeeAssetType === "transacting" ? "InTransactingAsset" : ""
      }`;
      const relayer_fee_decimals =
      ...
      const xcallParams = {
        ...
        slippage: (
          (typeof slippage === "number"
            ? slippage
            : DEFAULT_BRIDGE_SLIPPAGE_PERCENTAGE) * 100
        ).toString(),
        receiveLocal: receiveLocal || false,
        callData: callData || "0x",
        [relayer_fee_field]:
          relayerFee && Number(relayerFee) > 0
            ? utils
                .parseUnits(
                  Number(relayerFee).toFixed(relayer_fee_decimals),
      ...
      if (!failed) {
        const is_wrap_eth =
          ...
          ["ETH"].includes(source_asset_data?.symbol);

        try {
          if (is_wrap_eth) {
            xcallParams.asset = _source_contract_data?.contract_address;
            xcallParams.wrapNativeOnOrigin =
          ...
          if (
            ...
            )?.nativeCurrency?.symbol?.endsWith("ETH")
          ) {
            xcallParams.unwrapNativeOnDestination =
              xcallParams.receiveLocal || receive_wrap ? false : true;
          }
  ...
  // native asset
    ...
    (!symbol ||
      equalsIgnoreCase(destination_asset_data?.symbol, symbol) ||
      equalsIgnoreCase(destination_contract_data?.symbol, symbol))
  ) {
  ...
  const source_symbol =
    source_contract_data?.symbol || source_asset_data?.symbol;
  const source_amount = getBalance(
    source_chain_data?.chain_id,
    source_contract_data?.contract_address,
    balances_data
  )?.amount;

  const source_gas_native_token = _.head(
    source_chain_data?.provider_params
  )?.nativeCurrency;
  const source_gas_amount = getBalance(
    source_chain_data?.chain_id,
    constants.AddressZero,
    balances_data
  )?.amount;
  ...
  const destination_symbol =
    destination_contract_data?.symbol || destination_asset_data?.symbol;
  const destination_amount = getBalance(
    destination_chain_data?.chain_id,
    destination_contract_data?.contract_address,
    balances_data
  )?.amount;
  ...
  const router_fee = estimatedValues?.routerFee
    ? estimatedValues.routerFee
    : fees && (forceSlow ? 0 : fees.routerFee || 0);
        ...
        Number(
          utils.formatUnits(
            ...
            equalsIgnoreCase(
              ...
              ? destination_contract_data.next_asset?.decimals || 18
              : destination_decimals
          )
        )
      )
  ...
  const pool_data = toArray(pools_data).find(
    (p) => p?.chain_data?.id === destination_chain && p.asset_data?.id === asset
  );
  const { adopted, local } = { ...pool_data };
  const next_asset_data = adopted?.symbol?.startsWith(WRAPPED_PREFIX)
    ? adopted
    : local?.symbol?.startsWith(WRAPPED_PREFIX)
    ? local
    : local;

  const pool_amounts = toArray(_.concat(adopted, local))
    .filter((a) => ["string", "number"].includes(typeof a.balance))
    .map((a) => Number(a.balance));
  const pool_amount =
    receiveLocal || estimatedValues?.isNextAsset
      ? null
      : Number(next_asset_data?.balance) > -1
  ...
  const max_amount =
    source_amount &&
    utils.formatUnits(
      utils.parseUnits(source_amount, source_decimals).toBigInt() -
        utils
          .parseUnits(
            relayer_fee &&
              source_contract_data?.contract_address === constants.AddressZero
  ...
  const estimated_received = estimatedValues?.amountReceived
    ? estimatedValues.amountReceived
    : Number(amount) > 0 && typeof router_fee === "number"
  ...
  const estimated_time_seconds =
    (latest_transfer?.routers && latest_transfer.routers.length < 1) ||
    latest_transfer?.force_slow
      ? 5400
      : 240;
  const time_spent_seconds = moment().diff(
    moment(
  ...
  const errored =
    ...
    [XTransferStatus.XCalled, XTransferStatus.Reconciled].includes(
      latest_transfer.status
    );
  const has_latest_transfers =
    typeof latestTransfersSize === "number" && latestTransfersSize > 0;
  ...
  return (
    <div
      className={`grid grid-cols-1 ${
        has_latest_transfers ? "lg:grid-cols-8" : ""
      } items-start gap-4 my-4 sm:my-0 xl:my-4`}
    >
      <div className="hidden xl:block col-span-0 xl:col-span-2" />
      <div
        className={`col-span-1 ${
          has_latest_transfers ? "lg:col-span-5" : ""
        } xl:col-span-4`}
      >
        <div className="flex flex-col items-center justify-center mx-1 my-4 space-y-6 sm:space-y-6 sm:my-0 xl:my-6 sm:mx-4">
          <div
            className={`w-full ${
              openTransferStatus && latest_transfer ? "max-w-xl" : "max-w-md"
            } space-y-3`}
          >
            {openTransferStatus && latest_transfer ? (
              <div className="px-4 pt-5 pb-6 space-y-4 border rounded-2xl bg-slate-50 dark:bg-slate-900 dark:border-slate-700 sm:pt-5 sm:pb-6 sm:px-6">
                ...
                <div className="flex items-center justify-center">
                  <ActionRequired
                    ...
                    onTransferBumped={(relayer_fee_data) => {
                      if (latestTransfers) {
                        const index = latestTransfers.findIndex(
                          (t) =>
                            (t?.transfer_id &&
                              t.transfer_id === latest_transfer?.transfer_id) ||
                            (t?.xcall_transaction_hash &&
                              t.xcall_transaction_hash ===
                                latest_transfer?.transaction_hash)
                        );
                ...
                <div className="flex flex-col items-center space-y-2">
                  <span className="text-xs font-medium text-slate-500 dark:text-slate-500 sm:text-sm">
                    {latest_transfer.execute_transaction_hash ? (
                      <div className="flex flex-col items-center space-y-1">
                        <a
                          href={`${
                            destination_chain_data?.explorer?.url
                          }${destination_chain_data?.explorer?.transaction_path?.replace(
                            "{tx}",
                            latest_transfer.execute_transaction_hash
                          )}`}
                          target="_blank"
                      ...
                      <div className="flex flex-col items-center space-y-1">
                        {time_spent_seconds > estimated_time_seconds ? (
                          <span className="text-center">
                            Your assets are on the way! We will keep you
                            informed.
                          </span>
                        ) : (
                        ...
                        <span className="text-center">
                          If you close this window, your transaction will still
                          be processed.
                        </span>
                      </div>
                    )}
                    ...
                    <div className="space-y-7">
                      <div className="flex items-center justify-between space-x-2">
                        <h1 className="text-xl font-semibold">
                          Bridge
                          {receive_next && (
                            <span className="ml-1">into nextAsset</span>
                          )}
                        </h1>
                        {!["pool"].includes(source) && (
                          <Options
                            ...
                            onChange={(o) => {
                              ...
                              if (
                                (receiveLocal && !options?.receiveLocal) ||
                                (!receiveLocal && options?.receiveLocal)
                              ) {
                      ...
                      {chains_data && assets_data ? (
                        <div className="grid grid-cols-5 gap-3 sm:grid-cols-5 sm:gap-6">
                          <div className="col-span-2 sm:col-span-2 flex flex-col items-center sm:items-start space-y-0.5 sm:space-y-2">
                            <div className="w-32 sm:w-40 flex flex-col sm:flex-row sm:items-center justify-start space-x-1.5">
                              <span className="text-sm font-medium text-left text-slate-600 dark:text-slate-500">
                                From
                              </span>
                            </div>
                            <SelectChain
                          ...
                          <div className="flex items-center justify-center mt-5.5 sm:mt-7">
                            <button
                              ...
                              } rounded-2xl border flex items-center justify-center p-1 sm:p-1.5`}
                            >
                        ...
                        <div className="flex flex-col items-center justify-center h-64 space-y-3 sm:h-96">
                          <TailSpin
                            width="36"
                            height="36"
                    ...
                    {chains_data && assets_data && (
                      <>
                        <div className="space-y-2.5">
                          <div className="flex items-center justify-between space-x-2">
                            <div className="text-sm font-medium text-slate-600 dark:text-slate-500">
                              You send
                            </div>
                              ...
                              <div className="space-y-0">
                                <DebounceInput
                                  ...
                                  className={`w-36 sm:w-48 bg-transparent ${
                                    disabled ? "cursor-not-allowed" : ""
                                  } rounded-2xl border-0 focus:ring-0 sm:text-lg font-semibold text-right ${
                                    amount &&
                                    typeof source_asset_data?.price ===
                                      "number" &&
                        ...
                        asset &&
                        !checkSupport() ? (
                          <div className="font-medium text-center text-slate-400 dark:text-slate-200">
                            Route not supported
                          </div>
                        ) : (
                          checkSupport() && (
                            <div className="space-y-6">
                              <div className="space-y-2.5">
                                ...
                                <div className="bg-slate-100 dark:bg-slate-900 rounded-2xl border dark:border-slate-800 space-y-0.5 py-4 px-3">
                                  <div className="flex items-center justify-between space-x-2">
                                    <SelectAsset
                                      disabled={disabled}
                                      value={asset}
                                    ...
                                    [""].includes(amount) ||
                                    ["string", "number"].includes(
                                      typeof estimatedValues?.amountReceived
                                    ) ||
                                    estimateResponse ? (
                                ...
                                {!collapse && (
                                  <div className="space-y-2.5">
                                          ...
                                          {recipientEditing ? (
                                            <div className="flex items-center justify-end space-x-1.5">
                                              <DebounceInput
                                                ...
                                                className={`w-40 sm:w-56 bg-slate-100 focus:bg-slate-200 dark:bg-slate-800 dark:focus:bg-slate-700 rounded-2xl border-0 focus:ring-0 text-sm font-semibold text-right py-1.5 px-2`}
                                              />
                                              <button
                                                onClick={() =>
                                          ...
                                          (estimated_slippage > slippage ||
                                            ...
                                            <div className="flex items-start space-x-1">
                                              ...
                                              <div className="text-xs text-yellow-500 dark:text-yellow-400">
                                                ...
                                                slippage ? (
                                                  <>
                                                    Slippage tolerance is too
                                                    low
                                                    <br />
                                                    (use a larger amount or set
                                                    tolerance higher)
                                                  </>
                                                ) : slippage < 0.2 ? (
                                    ...
                                    <div className="flex items-center justify-between space-x-2">
                                      ...
                                      [""].includes(amount) ||
                                      ["string", "number"].includes(
                                        typeof estimatedValues?.routerFee
                                      ) ||
                                      estimateResponse ? (
                                      ...
                                      {fees ? (
                                        <div className="flex items-center space-x-1.5">
                                          <span className="whitespace-nowrap text-slate-500 dark:text-slate-500 text-sm font-semibold space-x-1.5">
                                            ...
                                            {is_staging ? (
                                              <select
                                                ...
                                                onChange={(e) => {
                                                  setRelayerFeeAssetType(
                                                    e.target.value
                                                  );
                                                  setEstimateFeesTrigger(
                                                    moment().valueOf()
                                                  );
                                                }}
                                                className="border-0 rounded-xl bg-slate-100 dark:bg-slate-800 focus:ring-0"
                                              >
                                                {RELAYER_FEE_ASSET_TYPES.map(
                                                  (t, i) => {
                                                    return (
                                                      <option
                                                        key={i}
                                          ...
                                          <button
                                            ...
                                            className="flex items-center justify-center rounded-full text-slate-400 hover:text-black dark:text-slate-200 dark:hover:text-white"
                                          >
                                            <MdRefresh size={16} />
                                          </button>
                                        </div>
                                    ...
                                    <div className="flex items-center justify-between space-x-2">
                                      <Tooltip
                                        placement="top"
                                        content={`Minimum amount received after slippage${
                                          typeof slippage === "number" &&
                                          slippage >= 0
                                            ? ` ${slippage}%`
                                            : ""
                                      ...
                                      >
                                        <div className="flex items-center">
                                          <div className="text-sm font-medium sm:whitespace-nowrap text-slate-500 dark:text-slate-500">
                                            Min amount received (with max
                                            slippage)
                                          </div>
                                          <BiInfoCircle
                                            size={14}
                    ...
                    (wrong_chain ||
                      ...
                      wrong_chain ? (
                        <Wallet
                          ...
                          className="w-full text-black bg-[#14f094] hover:bg-[#00CC66] dark:[#14f094] dark:hover:bg-[#00CC66]  rounded-xl flex items-center justify-center text-base font-medium space-x-1.5 sm:space-x-2 py-3 sm:py-4 px-2 sm:px-3"
                        >
                        ...
                        ![""].includes(amount) &&
                        ((utils
                          .parseUnits(amount || "0", source_decimals)
                          .toBigInt() >
                          utils
                            .parseUnits(source_amount || "0", source_decimals)
                            .toBigInt() &&
                          ["string", "number"].includes(
                            typeof source_amount
                          )) ||
                          Number(amount) < 0 ||
                          ...
                          (fees &&
                            (!relayer_fee || Number(relayer_fee) <= 0) &&
                            process.env.NEXT_PUBLIC_NETWORK !== "testnet") ||
                          (fees &&
                            Number(relayer_fee) > 0 &&
                            relayerFeeAssetType === "native" &&
                            source_gas_amount &&
                            utils.parseEther(source_gas_amount).toBigInt() <
                              utils.parseEther(relayer_fee).toBigInt() +
                                utils
                                  .parseEther(
                                    source_contract_data?.contract_address ===
                                      constants.AddressZero
                          ...
                          <span>
                            {utils
                              ...
                              utils
                                ...
                                .toBigInt() &&
                            ["string", "number"].includes(typeof source_amount)
                              ...
                              ? `Exceed Pool Balances: ${
                                  pool_amount >= 1000
                                    ? numberFormat(pool_amount, "0,0.00")
                                    : pool_amount
                                }`
                              : fees &&
                                (!relayer_fee || Number(relayer_fee) <= 0)
                        ...
                        (xcallResponse ||
                          ...
                          estimateResponse) &&
                        toArray(
                          xcallResponse || approveResponse || estimateResponse
                        ).map((r, i) => {
                      ...
                      <button
                        disabled={true}
                        className="w-full px-2 py-3 text-base text-center cursor-not-allowed rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 sm:py-4 sm:px-3"
                      >
                        Send
                      </button>
                    ) : (
                      <Wallet
                        ...
                        className="w-full px-2 py-3 text-base font-medium text-center rounded-xl  text-black bg-[#14f094] hover:bg-[#00CC66] dark:[#14f094] dark:hover:bg-[#00CC66]  sm:space-x-2 sm:py-4 sm:px-3"
                      >
      ...
      <div
        className={`col-span-1 ${
          has_latest_transfers ? "lg:col-span-3" : ""
        } xl:col-span-2`}
      >
        <LatestTransfers
